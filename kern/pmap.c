...
boot_alloc(uint32_t n)
{
	...
	if(n > 0) {
		...
		panic("Can't allocate negative bytes");
	}
	return result;
}
...
//	panic("x64_vm_init: this function is not finished\n");
	...
	// Your code goes here:
	int64_t distance = KSTKSIZE / PGSIZE;
	uint64_t bottom_address = KSTACKTOP-KSTKSIZE;
	boot_map_region(pml4e, bottom_address, KSTKSIZE, PADDR(bootstack), PTE_P | PTE_W);
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if((i*PGSIZE) == MPENTRY_PADDR) {
			// Do nothing
		} else if((i*PGSIZE) >= PGSIZE && (i*PGSIZE) < (npages_basemem * PGSIZE) && (i*PGSIZE) != MPENTRY_PADDR) {
			if(last) {
			...
			last = &pages[i];
		} else if(((i*PGSIZE) >= PADDR(BOOT_PAGE_TABLE_END)) && ((KADDR((physaddr_t)i*PGSIZE) >= boot_alloc(0) ))) {
			if(last) {
				last->pp_link = &pages[i];
			} else {
				page_free_list = &pages[i];
			...
			last = &pages[i];
		} else if(((i*PGSIZE) > EXTPHYSMEM) && ((i*PGSIZE) >= (uint64_t)boot_alloc(0))) {
			// TODO: avoid kernel, stuff above boot page table
			if(last) {
				last->pp_link = &pages[i];
			} else {
				page_free_list = &pages[i];
...
page_alloc(int alloc_flags)
{
	if(page_free_list == NULL) {
		...
		struct PageInfo *first_free = page_free_list;
		page_free_list = first_free->pp_link;
		first_free->pp_link = NULL;
		memset(page2kva(first_free), '\0', PGSIZE);
		return first_free; 
	} else {
		struct PageInfo *first_free = page_free_list;
		page_free_list = first_free->pp_link;
...
page_free(struct PageInfo *pp)
{
	...
	if((pp->pp_ref != 0) | (pp->pp_link != NULL)) {
		panic("Invalid call of page_free!");
	} else {
...
{
	uintptr_t index = PML4(va);
	// Check for present bit (0x001)
	int present = (pml4e[index] & PTE_P);
	if(present) {
		return pdpe_walk((pdpe_t *)KADDR(PTE_ADDR(pml4e[index])), va, create); 
	} else if(create) {
		struct PageInfo *new_page  = page_alloc(1);
		if(new_page == NULL) {
			return NULL;
		}
		// Need physical address of new table
		physaddr_t address = page2pa(new_page);
		pml4e[index] = address;
		pte_t *result = pdpe_walk((pdpe_t *)KADDR(PTE_ADDR(address)), va, create);
		// Remove the new page allocation if failed
		if(result == NULL) {
			page_free(new_page);
			return NULL;
		} else {
			pml4e[index] = pml4e[index] | PTE_USER;
			new_page->pp_ref += 1;
			return result;
		}
	} else {
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
	uintptr_t index = PDPE(va);
	// Check for present bit (0x001)
	int present = (pdpe[index] & PTE_P);
	if(present) {
		return pgdir_walk((pde_t *)KADDR(PTE_ADDR(pdpe[index])), va, create); 
	} else if(create) {
		struct PageInfo *new_page  = page_alloc(1);
		if(new_page == NULL) {
			return NULL;
		...
		physaddr_t address = page2pa(new_page);
		pdpe[index] = address;
		pte_t *result = pgdir_walk((pde_t *)KADDR(PTE_ADDR(address)), va, create);
		// Remove the new page allocation if failed
		if(result == NULL) {
			page_free(new_page);
			return NULL;
		} else {
			pdpe[index] = pdpe[index] | PTE_USER;
...
{
	uintptr_t index = PDX(va);
	// Check for present bit (0x001)
	int present = (pgdir[index] & PTE_P);
	if(present) {
		uintptr_t pte_index = PTX(va);
		pte_t *pte_address = KADDR(PTE_ADDR(pgdir[index]));
		return &pte_address[pte_index];
	} else if(create) {
		struct PageInfo *new_page = page_alloc(1);
		if(new_page == NULL) {
			return NULL;
		}
		// Need physical address of new page table entry
		physaddr_t address = page2pa(new_page);
		pgdir[index] = address | PTE_USER;
		new_page->pp_ref += 1;
		uintptr_t pte_index = PTX(va);
		pte_t *pte_address = KADDR(address);
		int entry_present = (pte_address[pte_index] & PTE_P);
		return &pte_address[pte_index];
	} else {
		return NULL;
	}
...
boot_map_region(pml4e_t *pml4e, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	for(int i = 0; i < (size / PGSIZE); i++) {
		uintptr_t current_address = la + (i*PGSIZE);
		pte_t *entry = pml4e_walk(pml4e,((const void * )la + i*PGSIZE), 1);
		*entry = ((pa + i*PGSIZE)| perm);
	}
}
...
page_insert(pml4e_t *pml4e, struct PageInfo *pp, void *va, int perm)
{
	...
	if(entry == NULL) {
		return -E_NO_MEM;
	} else if((*entry & PTE_P) == 1) {
		page_remove(pml4e, va);
		*entry = (page2pa(pp) | PTE_P | perm);
		pp->pp_ref += 1;
...
{
	pte_t *entry_address = pml4e_walk(pml4e, va, 0);
	if((entry_address != NULL) || (entry_address && PTE_P)) {
		if(pte_store != 0) {
			*pte_store = entry_address;
		}
		uint64_t index = PTX(va);
		return pa2page(PTE_ADDR(*entry_address));
	}else{
		return NULL;
	} 
}
...
{
	pte_t *myPTE; 
	struct PageInfo *target = page_lookup(pml4e, va, &myPTE);
	if(target != NULL) {
		page_decref(target);
		*myPTE = 0;
		tlb_invalidate(pml4e, va);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	size_t round_size = ROUNDUP(pa + size, PGSIZE);
	uint64_t address = ROUNDDOWN(pa, PGSIZE);
	round_size -= address;
	if(MMIOBASE + round_size > MMIOLIM) {
		panic("Overflowed MMIOLIM");
	} else {
		boot_map_region(boot_pml4e, base, round_size, address, PTE_W | PTE_PCD | PTE_PWT | PTE_P);
		base += round_size;
	}
	pte_t * entry = pml4e_walk(boot_pml4e, (void *)(base - round_size), 0);
...
{
	// LAB 3: Your code here.
	uint64_t va_floor, va_ceiling, base;
	pte_t * entry;
	va_floor = ROUNDDOWN((uint64_t) va, PGSIZE);
	base = ROUNDDOWN((uint64_t) va, PGSIZE);
	va_ceiling = ROUNDUP((uint64_t) va, PGSIZE);
	while(va_floor < va_ceiling) {
		if(va_floor >= ULIM) {
			user_mem_check_addr = ((uintptr_t) va);
			return -E_FAULT;
		}
		entry = pml4e_walk(env->env_pml4e, (void *) va_floor, 0);
		if((entry == NULL) || (*entry & perm) == 0) {
			if(va_floor == base) {
				user_mem_check_addr = ((uintptr_t) va);
			} else {
				user_mem_check_addr = ((uintptr_t) va_floor);
			}
			return -E_FAULT;
		} else {
			va_floor += PGSIZE;
	...
	if((((uint64_t)va + len) % PGSIZE) != 0) { 
		entry = pml4e_walk(env->env_pml4e, (void *) ((uint64_t)va + len), 0);
		if((entry == NULL) || (*entry & perm) == 0) {
			cprintf("Failing here\n");
			if(va_floor == base) {
				user_mem_check_addr = ((uintptr_t) va);
			} else {
				user_mem_check_addr = ((uintptr_t) va_floor);
