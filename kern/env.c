...
env_setup_vm(struct Env *e)
{
	...
	p->pp_ref++;	
	e->env_cr3 = PADDR(page2kva(p));
	// Use boot_pml4e to get pages, put them into environment
	// LOOK AT THE OFFSET IN THE VA FOR UTOP AND YOU CAN GET THE PML4E ENTRY
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	uint64_t round_va = ROUNDDOWN((uint64_t) va, PGSIZE);
	uint64_t upper_bound = ROUNDUP((uint64_t) va + len, PGSIZE);
	for(int i = 0; i < ((upper_bound - round_va)/PGSIZE); i++) {
		struct PageInfo * new_page = page_alloc(ALLOC_ZERO);
		int success = page_insert(e->env_pml4e, new_page, (void *)(round_va + PGSIZE*i), PTE_USER);
		if(success != 0) {panic("Region alloc failed!");}
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	if(header->e_magic != ELF_MAGIC) {
		panic("This isn't a valid ELF! We're all gonna die!");
	}
	...
	// Make sure run and pop_tf have stuff to look at
	e->env_status = ENV_RUNNABLE;
	// TODO: Set up the trapframe
	e->env_tf.tf_rip = (uintptr_t) header->e_entry;
	// Now map one page for the program's initial stack
	// at virtual address USTACKTOP - PGSIZE.
	// LAB 3: Your code here.
	struct PageInfo * stack_page = page_alloc(ALLOC_ZERO);
	page_insert(e->env_pml4e, stack_page, (void *) (USTACKTOP - PGSIZE), PTE_USER); 
	lcr3(boot_cr3);
}
...
env_run(struct Env *e)
{
	...
	if(e->env_status != ENV_RUNNABLE) {
		panic("Can't run an environment that isn't runnable!");
	}
